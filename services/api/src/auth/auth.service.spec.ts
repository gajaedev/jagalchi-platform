...
describe('AuthService', () => {
  ...
  it('redacts all authored Career data and disables the public profile on account deletion', async () => {
    const criteriaRows = [
      ...
      { type: ProofCriterionType.BaseBranch, config: { branch: 'private-branch' } },
      { type: ProofCriterionType.ChangedPath, config: { glob: 'private/**' } },
      { type: ProofCriterionType.NamedCheck, config: { context: 'Private check' } },
      { type: ProofCriterionType.HumanCheck, config: { label: 'Private human label' } },
    ];
    ...
    const repositories = new Map<unknown, Record<string, ReturnType<typeof vi.fn>>>();
    const repository = (
      entity: unknown,
    ...
    const claims = repository(GithubInstallationClaimAttempt, {
      delete: vi.fn(async ({ userId }: { userId: string }) => {
        claimRows.splice(0, claimRows.length, ...claimRows.filter((row) => row.userId !== userId));
      }),
    });
    ...
    const evidence = repository(CareerEvidence, {
      update: vi.fn(async (
        ...
        for (const row of evidenceRows) {
          ...
          if (where.reviewerId && row.reviewerId !== where.reviewerId) continue;
    ...
    const dataSource = {
      transaction: vi.fn(async (work: (value: typeof manager) => Promise<void>) =>
        work(manager),
      ),
    };
    const service = new AuthService(
    ...
    expect(installations.delete).toHaveBeenCalledWith({ ownerUserId: 'user-1' });
