...
the names of unresolved variables, never their values. The deploy command tags
API and AI images with the current Git SHA when Git metadata exists, otherwise
with a UTC timestamp. Before mutation it dumps the target
...
Application images are retained under their release SHA and each backup records
rollback image tags. Read
`.deploy-state/previous-release`, confirm both images exist with
`docker image ls`, then select them without rebuilding:

```bash
export API_IMAGE=jagalchi-personal-api:<previous-sha>
export AI_IMAGE=jagalchi-personal-ai:<previous-sha>
...
above. No GitHub, SSH, or Tailscale secret is stored on the VM.
